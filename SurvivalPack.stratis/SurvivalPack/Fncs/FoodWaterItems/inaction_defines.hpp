...
class SPCtrl_TreeView
		{
			...
			SizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 1)"; // Font from CfgFontFamilies
...
class SPCtrl_RscStructuredText
{
	deletable = 0;
	fade = 0;
	access = 0;
...
class SPCtrl_RscShortcutButton
{
	deletable = 0;
	fade = 0;
	type = 16;
...
class SPCtrl_RscShortcutButtonMain
{
	idc = -1;
	style = 0;
	default = 0;
...
class SPCtrl_RscButtonMenuCancel
{
	idc = 2;
	shortcuts[] = 
	{
...
class SPCtrl_RscProgress
{
	...
	type = CT_PROGRESS;
	style = ST_HORIZONTAL;
	colorFrame[] = {0.494,0.494,0.494,0.45};
	colorBar[] =
	{
		"(profilenamespace getvariable ['GUI_BCG_RGB_R',0.13])",
		"(profilenamespace getvariable ['GUI_BCG_RGB_G',0.54])",
